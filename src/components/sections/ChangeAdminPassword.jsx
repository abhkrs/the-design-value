...
function ChangeAdminPassword({ adminId = "" }) {
  const { closeChangePasswordModal } = useContext(LoginContext);
  ...
  const submitForm = async () => {
    if (username && password) {
      setshowLoader(true);
      const response = await api.post("/Register/changeAdmPw", {
        username,
        password,
      });
      if (response.stauts) {
      ...
      toast.error("Type user id and password please.", {
        autoClose: 3000,
        theme: "colored",
      });
    }
  ...
  return (
    <div className="bg-white p-2 rounded w-full relative lg:px-2 max-w-[95vw]">
      <div className="mb-5">
        <H3 className="!text-2xl mb-4">Update your password For {adminId}?</H3>
        <button
          type="button"
      ...
      <form className="w-full">
        ...
        <div className="relative">
          <input
            ...
            name="new-password"
            className="p-3 placeholder-secondary text-xl border-gray-200 rounded border w-full mt-3"
          ...
          <button
            ...
            className="absolute top-8 right-8 md:right-4 text-red-500"
          >
        ...
        >
          ...
          {!showLoader && <> Submit New Password</>}
        </button>
      </form>
    </div>
  );
