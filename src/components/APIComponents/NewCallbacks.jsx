...
export default function NewCallbacks() {
  ...
  const fetchActiveCallbacks = async () => {
    try {
      ...
      if (data.status === 'Success') {
        ...
        setActiveCallbacks(allCallbacks);
        setSearchResults(allCallbacks); // Set searchResults to allCallbacks initially
  ...
  const performSearch = (query) => {
    if (query === '') {
      setSearchResults(activeCallbacks); // Reset search results to all data
    } else {
      const filteredResults = activeCallbacks.filter(callback =>
